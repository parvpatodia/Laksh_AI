...
console.log("\nTest 4: 5 push-up cycles (elbow_angle signal)");
{
  ...
  for (let t = 0; t <= totalMs; t += dt) {
    const phase = (t % periodMs) / periodMs;
    // Elbow angle: top of push-up = ~170deg (arms straight); bottom = ~80deg
    const angleDeg = 125 - 45 * Math.cos(phase * 2 * Math.PI);
    // Synthesise shoulder-elbow-wrist with that angle. Place shoulder at
    // (0.5, 0.5), wrist at (0.5, 0.7) with elbow varying horizontally.
    // For angle theta at the elbow, with arms of length 0.1:
    const sx = 0.5, sy = 0.5;
    const wx = 0.5, wy = 0.7;
    const ex = 0.5;
    // Approximate elbow y so the angle ends up near angleDeg. With sym
    // setup, mid-y plus a bowed-out horizontal offset. We just need the
    // angle to MOVE with the right amplitude; precise geometry isn't
    // critical for the test.
    const ey = sy + 0.10 * (1 + 0.6 * Math.sin((angleDeg - 90) * Math.PI / 180));
    const vis = 0.9;
    const f = makeBlankFrame(vis);
    f[IDX.LEFT_SHOULDER] = { x: sx - 0.02, y: sy, z: 0, visibility: vis };
    f[IDX.RIGHT_SHOULDER] = { x: sx + 0.02, y: sy, z: 0, visibility: vis };
    f[IDX.LEFT_ELBOW] = { x: ex - 0.05, y: ey, z: 0, visibility: vis };
    f[IDX.RIGHT_ELBOW] = { x: ex + 0.05, y: ey, z: 0, visibility: vis };
