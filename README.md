...
| **D** — Gym MVP | Exercise v0 freeze, rep segmentation, per-rep feature vector with valid/degraded/unknown semantics | **Measurement spine complete**: [`app/gym/exercises_v0.py`](app/gym/exercises_v0.py) (12 movements, SHA-pinned manifest) + [`app/gym/rep_segmenter.py`](app/gym/rep_segmenter.py) (deterministic, per-rep status) + [`app/gym/rep_features.py`](app/gym/rep_features.py) (7-field per-rep vector, per-field status). Next: reference-range calibration config + labeled F1/IoU harness. See [GOALS.md](GOALS.md) Milestone 1. |
