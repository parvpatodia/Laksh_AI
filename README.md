...
| **D** — Gym MVP | Exercise v0 freeze, rep segmentation, per-rep feature vector with valid/degraded/unknown semantics, versioned reference-range calibration | **Milestone 1 measurement + calibration spine shipped**: [`app/gym/exercises_v0.py`](app/gym/exercises_v0.py) (12 movements, SHA-pinned manifest) + [`app/gym/rep_segmenter.py`](app/gym/rep_segmenter.py) (deterministic, per-rep status) + [`app/gym/rep_features.py`](app/gym/rep_features.py) (7-field per-rep vector) + [`app/gym/calibration_v0.py`](app/gym/calibration_v0.py) loading [`evaluation/gym_calibration_v0.json`](evaluation/gym_calibration_v0.json) (all 12 entries `uncalibrated_v0`; loader rejects silent ranges). Run `make verify-calibration-v0`. Next: end-to-end `analyze_gym_clip` script + labeled F1/IoU harness. See [GOALS.md](GOALS.md) Milestone 1. |
